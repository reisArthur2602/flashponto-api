...
def webhook():
    ...
    if stage == "awaiting_doc_observacao":
        ...
        conn.commit()
        cursor.execute("SELECT telefone FROM usuarios_rh")
    ...
    if stage == "awaiting_documento" and any(k in dados for k in ['file', 'document', 'media', 'image']):
        ...
        try:
            ...
            # -- Monta conjunto de telefones únicos --
            telefones_notificados = set()

            # SUPERVISOR (apenas o desse funcionário)
            if func.get('supervisor_id'):
                cursor.execute("SELECT telefone FROM usuarios_rh WHERE id=%s AND ativo=1", (func['supervisor_id'],))
                supervisor_row = cursor.fetchone()
                if supervisor_row:
                    logging.info(f"Notificando SUPERVISOR: {supervisor_row['telefone']}")
                    telefones_notificados.add(supervisor_row['telefone'])

            # RH e ADMIN (apenas esses tipos, não inclui supervisores!)
            cursor.execute("SELECT telefone FROM usuarios_rh WHERE tipo_usuario IN ('rh', 'admin') AND ativo=1")
            for row in cursor.fetchall():
                logging.info(f"Notificando RH/ADMIN: {row['telefone']}")
                telefones_notificados.add(row['telefone'])

            # Remove o telefone do próprio funcionário, caso ele também esteja em usuarios_rh
            telefones_notificados.discard(func['telefone'])

            # Notifica cada telefone uma única vez
            mensagem = f"📄 Documento #{doc_id} enviado por {func['nome']}. Aguarde análise/validação."
            for telefone in telefones_notificados:
                enviar_mensagem(telefone, mensagem)
                logging.info(f"Enviada notificação para: {telefone}")
